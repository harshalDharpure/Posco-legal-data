...
*Generation models (LLMs): main metric = ROUGE-1 F1. XLM-RoBERTa: classification metric = Accuracy (test set: 252 samples). NLI = entailment consistency (reference=premise, candidate=hypothesis); **NLI is now computed in the pipeline** — existing result JSONs were generated before NLI was added, so table shows "--" until evaluations are re-run (see below).*

| Model | Exp1 (FT only) | Exp2 (Pretrain only) | Exp3 (Pretrain+FT) | NLI Exp1 | NLI Exp2 | NLI Exp3 |
|-------|----------------|----------------------|--------------------|----------|----------|----------|
| LLaMA-3.1-8B | 0.4055 (R-1) | 0.2193 (R-1) | **0.4127** (R-1) | 0.5070 | 0.5315 | 0.4957 |
| Mistral-7B | 0.3998 (R-1) | 0.1639 (R-1) | 0.3968 (R-1) | 0.4790 | 0.2200 | 0.4557 |
| Qwen2.5-7B | 0.3582 (R-1) | 0.2167 (R-1) | 0.3609 (R-1) | 0.4604 | 0.3951 | 0.4858 |
| Qwen2.5-1.5B | -- | 0.1249 (R-1) | 0.3759 (R-1) | 0.3186 | 0.1947 | 0.4719 |
| Phi-3-mini | 0.2782 (R-1) | 0.1397 (R-1) | 0.2951 (R-1) | 0.4898 | 0.3430 | 0.4690 |
...
|-------|-------------------------|-------------------------|------------------------|----------|----------|----------|
| LLaMA-3.1-8B | 0.4055 | 0.2193 | **0.4127** | 0.5070 | 0.5315 | 0.4957 |
| Mistral-7B | 0.3998 | 0.1639 | 0.3968 | 0.4790 | 0.2200 | 0.4557 |
| Qwen2.5-7B | 0.3582 | 0.2167 | 0.3609 | 0.4604 | 0.3951 | 0.4858 |
| Qwen2.5-1.5B | -- | 0.1249 | 0.3759 | 0.3186 | 0.1947 | 0.4719 |
| Phi-3-mini | 0.2782 | 0.1397 | 0.2951 | 0.4898 | 0.3430 | 0.4690 |
| **XLM-RoBERTa-Large** | **0.9921** | **0.9881** | **1.0000** | -- | -- | -- |

**Note:** LLM values = ROUGE-1 F1. XLM-RoBERTa = Accuracy (classification). **To populate NLI:** NLI is computed in `evaluation/metrics.py` (`calculate_nli_score`) and saved when you run `models/evaluate_generation.py` (generation models). Re-run evaluation for each model/experiment to get `nli_score` in the result JSONs, then update this table from those JSONs. XLM-RoBERTa is classification-only (no generation), so NLI is left as "--" for that row.
...
| Qwen2.5-1.5B | -- | 0.3186 | 0.1947 | 0.4719 |
| XLM-RoBERTa-Large | -- | -- | -- | -- |
...
**NLI** (Natural Language Inference / entailment consistency) is **now implemented** in the pipeline: `evaluation/metrics.py` → `calculate_nli_score()` (DeBERTa MNLI; reference=premise, candidate=hypothesis). **Generation models:** `evaluate_generation.py` computes and saves `nli_score` in each result JSON. Existing result files were generated before NLI was added, so the table shows "--" until you **re-run evaluation** (e.g. `python3 models/evaluate_generation.py --model llama3.1_8b --experiment exp1` for each model/exp). Then copy `metrics.nli_score` from `models/<model>/results/exp*_results.json` into this document. **XLM-RoBERTa** is classification-only (no generation), so NLI stays "--".
